...
namespace CyberClubManager {
  internal class Program {
    ...
    private static void Main() {
      SessionNotificationService notificationService = new SessionNotificationService();
      AdminConsoleAlertObserver adminObserver = new AdminConsoleAlertObserver();

      notificationService.RegisterObserver(adminObserver);

      BookingController controller = new BookingController(DatabaseFilePath, notificationService);
    ...
    private static void HandleStartSession(BookingController controller) {
      ...
      if (!int.TryParse(Console.ReadLine(), out pcId)) {
        Console.WriteLine("Error: Invalid ID format.");
        return;
      }
    ...
    private static void HandleCloseSession(BookingController controller) {
      ...
      if (int.TryParse(Console.ReadLine(), out closeId)) {
